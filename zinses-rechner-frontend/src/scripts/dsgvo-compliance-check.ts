...
class DSGVOComplianceChecker {
  ...
  private async checkEdgeCases(): Promise<void> {
    ...
    const user = createAnonymousUser()
    // 设置为364天前创建（接近365天限制）
    user.createdAt = new Date(Date.now() - 364 * 24 * 60 * 60 * 1000)
...
async function main() {
  try {
    ...
    await checker.runFullComplianceCheck()
    process.exit(0)
  } catch (error) {
    console.error(colors.red('❌ DSGVO合规性检查失败:'))
    console.error(error)
    process.exit(1)
  }
}
