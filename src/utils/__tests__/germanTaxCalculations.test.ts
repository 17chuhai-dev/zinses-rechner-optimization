...
describe('germanTaxCalculations - 基础计算函数测试', () => {
  ...
  beforeEach(() => {
    defaultSettings = { ...DEFAULT_TAX_SETTINGS }
    basicParams = {
      income: 1000,
  ...
  describe('calculateETFTeilfreistellung 函数', () => {
    it('应该正确计算股票ETF的30%免税', () => {
      ...
      // 应税收入 = 1000 - 801 = 199
      // ETF免税 = 199 * 0.30 = 59.7
      expect(result.exemptAmount).toBeCloseTo(59.7, 1)
    ...
    it('应该正确计算房地产ETF的60%免税', () => {
      ...
      expect(result.exemptAmount).toBeCloseTo(119.4, 1) // 199 * 0.60
    })
  ...
  describe('calculateFreistellungsauftragUsage 函数', () => {
    ...
    it('应该考虑已使用的免税额度', () => {
      ...
      settingsWithUsedAllowance.freistellungsauftrag.usedAllowance = 300
      settingsWithUsedAllowance.freistellungsauftrag.remainingAllowance = 501
  ...
  describe('calculateTotalTaxBurden 函数', () => {
    it('应该正确计算总税负', () => {
      const result = calculateTotalTaxBurden(basicParams, defaultSettings)
    ...
    it('应该包含详细的税收分解', () => {
      ...
      expect(result.breakdown).toHaveProperty('baseTax')
      expect(result.breakdown).toHaveProperty('solidarityTax')
      expect(result.breakdown).toHaveProperty('churchTax')
      expect(result.breakdown).toHaveProperty('exemptAmount')
    })

    it('应该正确处理复杂的税收场景', () => {
      const complexSettings = { ...defaultSettings }
      complexSettings.userInfo.churchTaxType = ChurchTaxType.CATHOLIC
      complexSettings.abgeltungssteuer.churchTax.rate = 0.09
      complexSettings.abgeltungssteuer.calculation.includeChurchTax = true
      
      const complexParams = {
        ...basicParams,
        income: 5000,
        etfType: ETFType.REAL_ESTATE
      }
      
      const result = calculateTotalTaxBurden(complexParams, complexSettings)
      
      expect(result.totalTax).toBeGreaterThan(0)
      expect(result.breakdown.churchTax).toBeGreaterThan(0)
      expect(result.breakdown.exemptAmount).toBeGreaterThan(801) // 包含ETF免税
    })
  })
})

describe('germanTaxCalculations - 边界条件和特殊情况测试', () => {
  let defaultSettings: TaxSettings
  ...
  describe('边界值测试', () => {
    ...
    it('应该正确处理极大收入', () => {
      const largeIncomeParams: TaxCalculationParams = {
        income: 1000000,
        incomeType: 'capital_gains',
        jointFiling: false,
        deductions: 0,
      ...
      const result = calculateAbgeltungssteuer(largeIncomeParams, defaultSettings)
      
      expect(result.totalTax).toBeGreaterThan(0)
      expect(result.effectiveTaxRate).toBeCloseTo(0.26375, 3) // 约26.375%
    })
    ...
    it('应该正确处理略高于免税额度的收入', () => {
      const slightlyOverParams: TaxCalculationParams = {
        income: 802,
        incomeType: 'capital_gains',
        jointFiling: false,
        deductions: 0,
      ...
      const result = calculateAbgeltungssteuer(slightlyOverParams, defaultSettings)
      
      expect(result.taxableIncome).toBe(1)
      expect(result.baseTax).toBe(0.25) // 1 * 0.25
    })
  })

  describe('精度和舍入测试', () => {
    it('应该正确处理小数计算', () => {
      const decimalParams: TaxCalculationParams = {
        income: 1000.33,
        incomeType: 'capital_gains',
        jointFiling: false,
        deductions: 0,
      ...
      const result = calculateAbgeltungssteuer(decimalParams, defaultSettings)
      
      expect(result.totalTax).toBeCloseTo(52.61, 2) // 精确到分
    })

    it('应该使用正确的舍入方法', () => {
      const result = roundToDecimalPlaces(1.235, 2)
      expect(result).toBe(1.24) // 四舍五入
      
      const result2 = roundToDecimalPlaces(1.234, 2)
  ...
  describe('错误处理测试', () => {
    it('应该处理负收入', () => {
      ...
      expect(result.totalTax).toBe(0)
      expect(result.netIncome).toBe(-1000) // 保持原值
    })

    it('应该处理无效的ETF类型', () => {
      const invalidETFParams: TaxCalculationParams = {
        income: 1000,
        incomeType: 'capital_gains',
        etfType: 'invalid_type' as ETFType,
        jointFiling: false,
        deductions: 0,
        taxYear: 2024
      }
      
      const result = calculateETFTeilfreistellung(invalidETFParams, defaultSettings)
...
describe('germanTaxCalculations - 工具函数测试', () => {
  ...
  describe('optimizeFreistellungsauftragAllocation 函数', () => {
    it('应该优化免税额度分配', () => {
      const settings = { ...DEFAULT_TAX_SETTINGS }
      settings.freistellungsauftrag.allocations = [
        {
          id: '1',
          bankName: 'Bank A',
          allocatedAmount: 400,
      ...
      const optimized = optimizeFreistellungsauftragAllocation(settings)
      
      expect(optimized).toHaveLength(2)
      expect(optimized[0].allocatedAmount).toBeGreaterThan(0)
    })
